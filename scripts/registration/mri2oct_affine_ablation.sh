...
OUT_DIR="$ROOT/mri2oct/affine_vessels-and-segment"
...
OUT_DIR="$ROOT/mri2oct/affine_no-vessels"
nitorch register2 --verbose 4 --gpu --output-dir $OUT_DIR \
	"${LOSS_IMAGE[@]}" "${LOSS_SEGMENT[@]}" \
	@affine affine --progressive \
	@@optim gn --tolerance 1e-8 -n 256 \
	@pyramid --levels 1:5

OUT_DIR="$ROOT/mri2oct/affine_no-segment"
nitorch register2 --verbose 4 --gpu --output-dir $OUT_DIR \
	"${LOSS_IMAGE[@]}" "${LOSS_VESSELS[@]}" \
	@affine affine --progressive \
	@@optim gn --tolerance 1e-8 -n 256 \
	@pyramid --levels 1:5

OUT_DIR="$ROOT/mri2oct/affine_no-vessels_no-segment"
nitorch register2 --verbose 4 --gpu --output-dir $OUT_DIR \
	"${LOSS_IMAGE[@]}"  \
	@affine affine --progressive \
	@@optim gn --tolerance 1e-8 -n 256 \
